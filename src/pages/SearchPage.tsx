...
import { CalendarX2, ArrowRight, Wifi, Wind, Waves, Coffee, Check } from 'lucide-react';
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredUnits.map((unit) => (
                <Card key={unit.id} className="group hover:shadow-md transition-all duration-300">
                  <div className="relative h-56 overflow-hidden">
                    <img 
                      src={getImageUrl(unit.type, unit.id)} 
                      alt={unit.name}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                      referrerPolicy="no-referrer"
                    />
                    <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold tracking-wider uppercase text-zinc-900 shadow-sm">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <div className="mb-4">
                      <h3 className="text-xl font-semibold text-zinc-900 mb-1 tracking-tight">{unit.name}</h3>
                      <p className="text-sm text-zinc-500">Capacidad: hasta {unit.capacity} personas</p>
                    </div>

                    <div className="mt-auto pt-4 border-t border-zinc-100 flex items-center justify-between">
                      <div>
                        <span className="text-2xl font-bold text-zinc-900">{unit.base_price}€</span>
                        <span className="text-sm text-zinc-500"> / noche</span>
                      </div>
                      <button 
                        onClick={() => window.location.href = `/?tab=book&id=${unit.id}&checkIn=${checkIn}&checkOut=${checkOut}`}
                        className="px-5 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-full text-sm font-medium transition-colors flex items-center gap-2"
                      >
                        Ver detalles
                      </button>
                    </div>
